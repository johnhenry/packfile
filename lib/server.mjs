...
import { representsDir, representsFile } from "./represents.mjs";
...
export const compileDirectory = async (directoryPath, options = {}) => {
  const {
    compress = true,
    compressionLevel = constants.Z_DEFAULT_COMPRESSION,
    ignorePatterns = [],
  ...
  const readDirectoryRecursive = async (currentPath) => {
    ...
    for (const entry of entries) {
      ...
      if (
        entry.isDirectory() ||
        (entry.isSymbolicLink() && representsFile(entry))
      ) {
        await readDirectoryRecursive(fullPath);
      } else if (entry.isFile() && representsDir(entry)) {
  ...
  try {
    ...
    if (compress) {
      compiledData = await compressObject(compiledData, compressionLevel);
    }
...
const getContentType = (filePath, customMimeTypes) => {
